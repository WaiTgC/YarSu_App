...
//     const url = "https://www.tiktok.com/@yarsu1718?_t=ZS-8xvaxitAr3Q&_r=1";
//     try {
//       const supported = await Linking.canOpenURL(url);
//       if (supported) {
//         await Linking.openURL(url);
...
//     const url = "https://www.youtube.com/@nannan-st6on";
//     try {
//       const supported = await Linking.canOpenURL(url);
//       if (supported) {
//         await Linking.openURL(url);
...
//     const url =
//       "https://www.facebook.com/profile.php?id=100064695905923&mibextid=wwXIfr&rdid=s7eCkatYqaEeCSiB#";
//     try {
//       const supported = await Linking.canOpenURL(url);
//       if (supported) {
//         await Linking.openURL(url);
...
export default function Home({ toggleSidebar }: HomeProps) {
  ...
  const handleTelegramPress = async () => {
    const url = profile.telegram || "https://t.me"; // Fallback to generic Telegram
    if (url) {
      try {
        const supported = await Linking.canOpenURL(url);
        if (supported) {
  ...
  const handleTikTokPress = async () => {
    const url = profile.tiktok || "https://tiktok.com"; // Fallback to generic TikTok
    if (url) {
      try {
        const supported = await Linking.canOpenURL(url);
        if (supported) {
  ...
  const handleYouTubePress = async () => {
    const url = profile.youtube || "https://youtube.com"; // Fallback to generic YouTube
    if (url) {
      try {
        const supported = await Linking.canOpenURL(url);
        if (supported) {
  ...
  const handleFacebookPress = async () => {
    const url = profile.facebook || "https://facebook.com"; // Fallback to generic Facebook
    if (url) {
      try {
        const supported = await Linking.canOpenURL(url);
        if (supported) {
