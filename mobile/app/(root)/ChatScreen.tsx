...
export default function ChatScreen({ route }: any) {
  ...
  useEffect(() => {
    // Fetch userId from SecureStore (assume it's stored as 'userId' after login)
    import('expo-secure-store').then(SecureStore => {
      SecureStore.getItemAsync('userId').then(id => setUserId(id));
    });
    fetchMessages();
    // Poll in the background, but update messages without setting loading state
    ...
    >
      <View style={styles.container}>
        ...
        <View style={styles.inputRowFixed}>
          ...
          />
          <Button title={sending ? 'Sending...' : 'Send'} onPress={sendMessage} disabled={sending} />
        </View>
      </View>
    </KeyboardAvoidingView>
  );
